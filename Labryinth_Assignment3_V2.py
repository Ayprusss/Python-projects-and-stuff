...
# listToString() convertes a given list  of char and int to a string to be ready to print out
def listToString(list):
    #Concatenate list's elements into a string str
    for ele in list:
        list2Str = ''.join([str(element) for element in list])
    #return list2Str
    return list2Str
...
print('exit: ', exit)
######## Converte this into function createZeroMaze() ####
...
def traceback(mypath, exitR, exitC): # r and c are current EXIT point. (row and columns of selected cell.);  ### 
    ...
    while p >= 1:
        ...
        # At the exist cell on column 15, the exist cell always has ">" direction to exist the maze.
        # Similar to the entrance cell on column 0, the entrance cell always has ">" direction
                
        if (p== 15 or p == 0):
            cellDirectionStr = '>'
        
        newCell = populateCell(mypath, returnPathList, newR, newC, "*") # create cell object of push into path coordinate. 
        newCell.setPrintValue(cellDirectionStr)
...
for e in range( len(myFinalMaze)):
    print(listToString(myFinalMaze[e]))
